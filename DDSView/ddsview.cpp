...
class SourceImage : public TexMetadata
{
public:
    HRESULT Load(LPWSTR filePath)
    {
        m_filePath = filePath;

        HRESULT hr = GetMetadataFromDDSFile(filePath, DDS_FLAGS_NONE, *this);

        if (FAILED(hr))
        {
            wchar_t buff[2048] = {};
            swprintf_s(buff, L"Failed to open texture file\n\nFilename = %ls\nHRESULT %08X", filePath, hr);
            MessageBox(nullptr, buff, L"DDSView", MB_OK | MB_ICONEXCLAMATION);
            return hr;
        }

        hr = LoadFromDDSFile(filePath, DDS_FLAGS_NONE, this, m_image);
        if (FAILED(hr))
        {
            wchar_t buff[2048] = {};
    ...
    SIZE GetSize() const
    {
        ...
        size.cx = LONG(width);
        size.cy = LONG(height);
        const bool isCubeMap = (miscFlags & TEX_MISC_TEXTURECUBE) != 0;
        if (isCubeMap)
        {
            size.cx *= 4;
            size.cy *= 2;
    ...
    HRESULT CopyTo(ID3D11Device* pDevice, ID3D11ShaderResourceView** pSRV)
    {
        TexMetadata mdata = *static_cast<TexMetadata*>(this);

        // Special case to make sure Texture cubes remain arrays
        mdata.miscFlags &= ~TEX_MISC_TEXTURECUBE;

        HRESULT hr = CreateShaderResourceView(pDevice, m_image.GetImages(), m_image.GetImageCount(), mdata, pSRV);

        if (FAILED(hr))
        {
            wchar_t buff[2048] = {};
            swprintf_s(buff, L"Failed creating texture from file\n\nFilename = %ls\nHRESULT = %08X", m_filePath, hr);
            MessageBox(nullptr, buff, L"DDSView", MB_OK | MB_ICONEXCLAMATION);
            return hr;
        }
...
private:
    LPWSTR m_filePath = nullptr;
    ScratchImage m_image;
};

HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow, TexMetadata& mdata);
HRESULT InitDevice(const SourceImage& mdata);
void CleanupDevice();
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    SourceImage mdata;
    if (FAILED(mdata.Load(lpCmdLine)))
        return 1;
    ...
    if (FAILED(mdata.CopyTo(g_pd3dDevice, &g_pSRV)))
        return 1;

    SetWindowLongPtr(g_hWnd, GWLP_USERDATA, LONG_PTR(&mdata));
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        auto source = reinterpret_cast<SourceImage*>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
        ...
        if (source)
        {
            ...
            if (SUCCEEDED(InitDevice(*source)))
            {
                source->CopyTo(g_pd3dDevice, &g_pSRV);
            }
        }
...
HRESULT InitDevice(const SourceImage& mdata)
{
    ...
    static const SimpleVertex verticesCube[] =
    {
        ...
        // XPOS
        { XMFLOAT4(.5f, .25f, 0.f, 1.f),   XMFLOAT4(0.f, 0.f, 5.f, 0.f) },
        { XMFLOAT4(1.f, .25f, 0.f, 1.f),   XMFLOAT4(1.f, 0.f, 5.f, 0.f) },
        { XMFLOAT4(.5f, -.25f, 0.f, 1.f),  XMFLOAT4(0.f, 1.f, 5.f, 0.f) },
        { XMFLOAT4(1.f, -.25f, 0.f, 1.f),  XMFLOAT4(1.f, 1.f, 5.f, 0.f) },

        // XNEG
        { XMFLOAT4(-.5f, .25f, 0.f, 1.f),  XMFLOAT4(0.f, 0.f, 4.f, 0.f) },
        { XMFLOAT4(0.f, .25f, 0.f, 1.f),   XMFLOAT4(1.f, 0.f, 4.f, 0.f) },
        { XMFLOAT4(-.5f, -.25f, 0.f, 1.f), XMFLOAT4(0.f, 1.f, 4.f, 0.f) },
        { XMFLOAT4(0.f, -.25f, 0.f, 1.f),  XMFLOAT4(1.f, 1.f, 4.f, 0.f) },
        ...
        // ZPOS
        { XMFLOAT4(0.f, .25f, 0.f, 1.f),   XMFLOAT4(0.f, 0.f, 0.f, 0.f) },
        { XMFLOAT4(.5f, .25f, 0.f, 1.f),   XMFLOAT4(1.f, 0.f, 0.f, 0.f) },
        { XMFLOAT4(0.f, -.25f, 0.f, 1.f),  XMFLOAT4(0.f, 1.f, 0.f, 0.f) },
        { XMFLOAT4(.5f, -.25f, 0.f, 1.f),  XMFLOAT4(1.f, 1.f, 0.f, 0.f) },

        // ZNEG
        { XMFLOAT4(-1.f, .25f, 0.f, 1.f),  XMFLOAT4(0.f, 0.f, 1.f, 0.f) },
        { XMFLOAT4(-.5f, .25f, 0.f, 1.f),  XMFLOAT4(1.f, 0.f, 1.f, 0.f) },
        { XMFLOAT4(-1.f, -.25f, 0.f, 1.f), XMFLOAT4(0.f, 1.f, 1.f, 0.f) },
        { XMFLOAT4(-.5f, -.25f, 0.f, 1.f), XMFLOAT4(1.f, 1.f, 1.f, 0.f) },
    };
...
void CleanupDevice()
{
    ...
    SafeRelease(g_pCBArrayControl);
    SafeRelease(g_pSRV);
    SafeRelease(g_AlphaBlendState);
